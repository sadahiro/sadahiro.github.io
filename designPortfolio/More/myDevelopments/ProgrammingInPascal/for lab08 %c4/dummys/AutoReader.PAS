...
{Lab number:		-08					Due Date:	Friday April 9th '93}

{Purpose:		read source program and rewrite another file with some modification execution}

{Input :	FnameIN, FnameOUT:string;	Uname:string;}

{Outputs :	FileOUT:text}

{comment:	since mac and ibm is so different, i couldnt use some of functions for ibm. }
{			I used functions which does similar to ibm's, but i sometimes had to chose one }
{					which is so different . }
{			there was nothing for me to do for right now.  my knowledge for mac is still so poor........}
...
program AutoReader (input, output, datafile);
	...
	procedure Header (var FileOUT: text;
									FnameIN, FnameOUT: str255);
		var
			Uname: str255;
	begin
	...
	procedure Conclusion (var FileIN, FileOUT: text;
	...
	begin
		...
		close(PF)
	end;				{end of printfile}

	procedure Execution (var FileIN, FileOUT: text;
									FnameIN, FnameOUT: str255);
		var
			Line, UnLine: str255;
			LineLength: integer;
